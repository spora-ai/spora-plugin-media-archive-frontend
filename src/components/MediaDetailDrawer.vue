...
<template>
    <div
        class="fixed inset-0 z-40 flex justify-end bg-foreground/30 backdrop-blur-sm"
        role="dialog"
        aria-modal="true"
        data-testid="media-drawer-backdrop"
    ...
    >
        ...
        >
            ...
            >
                <img :src="asset.asset_url" :alt="asset.prompt ?? 'Archived image'" class="h-auto w-full" />
            </figure>
            <audio
            ...
            <p v-if="asset.prompt" class="rounded-md bg-muted/60 p-3 text-sm text-foreground">
                {{ asset.prompt }}
            </p>
        </aside>
    </div>
